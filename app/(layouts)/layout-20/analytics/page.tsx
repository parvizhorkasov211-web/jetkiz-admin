...
function MetricCard({
  ...
  return (
    <Comp
      type={onClick ? 'button' : undefined}
      onClick={onClick}
      className="rounded-2xl border border-gray-200 bg-white px-4 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
...
export default function AnalyticsPage() {
  ...
  const loadAnalytics = async (
    ...
    try {
      ...
      params.set('period', nextPeriod === 'custom' ? '30d' : nextPeriod);
      ...
      const [financeJson, restaurantJson, courierJson] = await Promise.all([
        apiFetch(`/finance/summary${suffix}`, {
          method: 'GET',
          cache: 'no-store',
        }),
        apiFetch(`/finance/restaurant-payouts/summary${suffix}`, {
          method: 'GET',
          cache: 'no-store',
        }),
        apiFetch(`/finance/courier-payouts/summary${suffix}`, {
          method: 'GET',
          cache: 'no-store',
        }),
      ]);

      setData(financeJson as FinanceSummary);
      setRestaurantData(restaurantJson as RestaurantPayoutSummary);
      setCourierData(courierJson as CourierPayoutSummary);
    } catch (e) {
              ...
              <div className="rounded-2xl border border-gray-100 bg-gray-50 p-3">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setPeriod('custom');
                      void loadAnalytics('custom', fromDate, toDate);
                    }}
