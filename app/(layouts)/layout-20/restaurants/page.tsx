...
type StatusFilter = 'ALL' | 'OPEN' | 'CLOSED' | 'OFFLINE';

type PendingRestaurantIdsResponse = {
  ids?: string[];
};

type DisplayStatus = 'OPEN' | 'CLOSED' | 'OFFLINE';

/**
 * Контекст страницы:
 * - это текущая production-страница ресторанов в админке
 * - здесь сохранена существующая логика:
 *   поиск, фильтры, toggle isInApp, override комиссии, удаление, переход в details page
 * - добавлена новая логика для restaurant registration flow:
 *   1) если isInApp=false => статус OFFLINE
 *   2) NEW badge для новых ресторанов, пришедших через регистрацию
 *
 * Backend endpoints, которые использует эта страница:
 * - GET    /restaurants
 * - GET    /restaurants/commission/default
 * - PATCH  /restaurants/commission/default
 * - PATCH  /restaurants/:id/in-app
 * - PATCH  /restaurants/:id/commission
 * - POST   /restaurants/:id/commission/reset
 * - DELETE /restaurants/:id
 * - GET    /restaurant-auth/admin/pending-ids
 *
 * ВАЖНО:
 * - NEW badge берётся не из выдуманного поля, а из отдельного backend endpoint,
 *   который возвращает id ресторанов, пришедших через registration flow и ещё не включённых в приложение.
 */

function getDisplayStatus(row: RestaurantRow): DisplayStatus {
  if (!row.isInApp) return 'OFFLINE';
  return row.runtimeStatus === 'OPEN' ? 'OPEN' : 'CLOSED';
}

function StatusPill({ row }: { row: RestaurantRow }) {
...
function NewRestaurantBadge() {
  return (
    <span
      className="inline-flex items-center gap-2 px-2.5 py-1 rounded-md border border-amber-200 bg-amber-50 text-amber-700 font-extrabold text-[12px]"
      title="Новый ресторан, отправивший заявку на подключение"
    >
...
export default function RestaurantsPage() {
  ...
  // ======================================================
  // NEW RESTAURANTS (из registration flow)
  // ======================================================
  ...
  /**
   * В query отправляем только то, что реально поддерживает backend /restaurants:
   * - q
   * - status=OPEN|CLOSED
   *
   * OFFLINE делаем на frontend, потому что это логика от isInApp=false,
   * а не runtimeStatus.
   */
  const query = useMemo(() => {
    const sp = new URLSearchParams();
    if (q) sp.set('q', q);
    if (status === 'OPEN' || status === 'CLOSED') sp.set('status', status);
    return sp.toString();
  }, [q, status]);

  // ======================================================
  // LOAD: restaurants + pending new restaurant ids
  // ======================================================
  useEffect(() => {
    let alive = true;
    ...
    Promise.all([
      ...
      apiFetch<PendingRestaurantIdsResponse>(`/restaurant-auth/admin/pending-ids`, {
        method: 'GET',
      }).catch(() => ({ ids: [] })),
    ])
      .then(([data, pending]) => {
        ...
        setItems(list);
        setNewRestaurantIds(Array.isArray(pending?.ids) ? pending.ids : []);
  ...
  /**
   * OFFLINE фильтруем уже по загруженным данным,
   * потому что OFFLINE = isInApp === false.
   */
  const visibleItems = useMemo(() => {
    if (status !== 'OFFLINE') return items;
    return items.filter((x) => !Boolean(x.isInApp));
  }, [items, status]);

  const newRestaurantSet = useMemo(() => new Set(newRestaurantIds), [newRestaurantIds]);
  ...
  const handleToggleInApp = async (id: string, next: boolean) => {
    try {
      ...
      /**
       * Если ресторан включили в приложение,
       * он больше не должен считаться NEW в админке.
       */
      if (newVal === true) {
        setNewRestaurantIds((prev) => prev.filter((x) => x !== id));
      }
    } catch (e: any) {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <option value="OFFLINE">OFFLINE (не в приложении)</option>
          </select>
        ...
        <table className="min-w-[2000px] w-full text-[15px] table-auto">
          <thead className="bg-gray-100 border-b border-gray-300">
            <tr>
              ...
              <th className="text-left p-4 font-bold text-gray-900 w-[190px]">Статус</th>
          ...
          <tbody>
            {visibleItems.map((x, index) => {
              ...
              const isNewRestaurant = newRestaurantSet.has(x.id);
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 font-extrabold text-gray-900 text-[16px]">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span>{x.nameRu}</span>
                      {isNewRestaurant && <NewRestaurantBadge />}
                    </div>
                  </td>
