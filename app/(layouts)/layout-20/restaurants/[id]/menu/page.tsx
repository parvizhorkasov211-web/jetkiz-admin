...
function normalizeMenuResponse(raw: any): MenuResponse {
  ...
  return {
    restaurant: raw?.restaurant || { id: '', nameRu: '' },
    categories,
    items,
    products: items,
  };
...
export default function RestaurantMenuPage() {
  ...
  const loadMenu = async () => {
    try {
      ...
      const raw = await apiFetch(`/restaurants/${id}/menu?includeUnavailable=1`, {
        cache: 'no-store',
      });

      const json = normalizeMenuResponse(raw);
  ...
  const menuCategories = useMemo(() => data?.categories ?? [], [data]);
  ...
  const activeCategoryTitle = useMemo(() => {
    ...
    return menuCategories.find((c) => c.id === activeCategory)?.titleRu || 'Категория';
  }, [data, activeCategory, menuCategories]);

  const items = useMemo(() => {
    if (!data) return [];
    if (!activeCategory) return menuItems;
    return menuItems.filter((i) => i.categoryId === activeCategory);
  }, [data, activeCategory, menuItems]);
  ...
  try {
    ...
    try {
      await apiFetch(`/restaurants/${id}/categories/${category.id}`, {
        method: 'DELETE',
      });
    } catch (e: any) {
    ...
    if (msg.includes('409')) {
      try {
        const r = await fetch(
          `/api/proxy/restaurants/${id}/categories/${category.id}`,
          {
            method: 'DELETE',
            credentials: 'include',
            cache: 'no-store',
  ...
  const refreshEditImagesFromMenu = async (productId: string) => {
    ...
    const next = normalizeMenuResponse(raw);

    setData(next);

    const nextItems = next.items ?? next.products ?? [];
  ...
  const goEditItem = (itemId: string) => {
    const item = menuItems.find((x) => x.id === itemId) || null;
    if (!item) return;
